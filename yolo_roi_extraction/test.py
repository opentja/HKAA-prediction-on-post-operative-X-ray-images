...
def test(cfg,
    ...
    for batch_i, (imgs, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        with torch.no_grad():
            ...
            ''' 
            print('train_out[0] shape: ', train_out[0].shape)
            print('train_out[1] shape: ', train_out[1].shape)
        ...
        for si, pred in enumerate(output):
            ...
            if save_json:
                ...
                image_id = int(Path(paths[si]).stem.split('_')[-1].replace('.',''))
                #print(image_id)
    ...
    if save_json and map and len(jdict):
        imgIds = [int(Path(x).stem.split('_')[-1].replace('.','')) for x in dataloader.dataset.img_files]
...
if __name__ == '__main__':
    ...
    opt.save_json = False #or any([x in opt.data for x in ['coco.data', 'coco2014.data', 'coco2017.data']])
    print(opt)
    ptName = 'batchSize' + str(opt.batch_size) +'Epochs' + str(opt.epochs) + 'LR' + str(opt.lr) +'-'+str(opt.count) +'best.pt'
    print(ptName, opt.task)
